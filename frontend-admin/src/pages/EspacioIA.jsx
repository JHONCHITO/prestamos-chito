...
function loadConversationId() {
  const stored = sessionStorage.getItem('admin_workspace_conversation_id');
  if (stored) return stored;
  const next = createConversationId();
  sessionStorage.setItem('admin_workspace_conversation_id', next);
  return next;
}
...
export default function EspacioIA() {
  ...
  async function loadDocuments(includeInactive = showArchivedDocuments) {
    ...
    try {
      const response = await ragAPI.documents({
        limit: 14,
  ...
  async function loadMessages(id) {
    ...
    try {
      const response = await ragAPI.conversationMessages(id);
      const payload = response.data || {};
  ...
  const updateDocumentState = async (item, action) => {
    ...
    try {
      if (action === 'archive') {
        const response = await ragAPI.archiveDocument(item.sourceId, {
          targetTenantId: tenantId || undefined,
        });
        message.success(response.data?.message || 'Documento archivado');
      } else if (action === 'restore') {
        const response = await ragAPI.restoreDocument(item.sourceId, {
          targetTenantId: tenantId || undefined,
        });
        message.success(response.data?.message || 'Documento restaurado');
      } else if (action === 'delete') {
        const response = await ragAPI.deleteDocument(item.sourceId, {
          targetTenantId: tenantId || undefined,
        });
        message.success(response.data?.message || 'Documento eliminado');
      }
  ...
  const openConversation = (id) => {
    ...
    sessionStorage.setItem('admin_workspace_conversation_id', id);
  };

  const resetConversation = () => {
    const nextConversationId = createConversationId();
    sessionStorage.setItem('admin_workspace_conversation_id', nextConversationId);
